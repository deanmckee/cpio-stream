...
var END_OF_CPIO = {name: 'TRAILER!!!', nlink: 1}
...
function Pack (opts) {
  ...
  } else if(opts.format === 'crc') {
    this.newc = opts
    this._encode = require('./crc').encode
  } else {
...
Pack.prototype.entry = function (header, buffer, callback) {
  ...
  if (Buffer.isBuffer(buffer)) {
    ...
    if (this.newc) {
      ...
      this._push(new Buffer(new Array(paddinglength + 1).join('\0')))
    }
  ...
  eos(sink, function (err) {
    ...
    if (self.newc) {
      ...
      var paddinglength = (4 - mod4) % 4
      self._push(new Buffer(new Array(paddinglength + 1).join('\0')))
    }
